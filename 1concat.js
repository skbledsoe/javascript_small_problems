/*
  ...
  algo:
    given an array and a second element
      create new concatArr

      loop through first array, pushing each element into the concatArr

      if second element isArray,
        loop through and push each element into concatArr

      otherwise,
        push element into concatArr
...
function concat(array1, secondArgument) {
  ...
  for (let idx = 0; idx < array1.length; idx++) {
    concatArr.push(array1[idx]);
  }

  if (Array.isArray(secondArgument)) {
    for (let idx = 0; idx < secondArgument.length; idx++) {
      concatArr.push(secondArgument[idx]);
    }
  } else {
    concatArr.push(secondArgument);
  }
